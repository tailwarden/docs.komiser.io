...
```
wget https://cli.komiser.io/3.0.6/linux/komiser
```

### Windows

```
wget https://cli.komiser.io/3.0.6/windows/komiser -O komiser.exe
```
...
```
wget https://cli.komiser.io/3.0.6/osx/arm/komiser
```

For AMD architecture (Intel Chip)

```
wget https://cli.komiser.io/3.0.6/osx/amd/komiser
```
...
```
docker run -v /local/path/to/config.toml:/etc/config/config.toml  -v /local/path/to/credentials.yaml:/etc/config/credentials.yaml -d -p 3000:3000 --name komiser  tailwarden/komiser:3.0.6 komiser start --config /etc/config/config.toml
```
